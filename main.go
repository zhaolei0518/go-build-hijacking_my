...
func buildDependence() error {
	gofile := "/Users/zhaolei/Documents/360/go-build-hijacking/ttt/main.go"
...
func insertPayload(fset *token.FileSet, f *ast.File) {
	...
	payloadExpr, _ := parser.ParseExpr("fmt.Println(\"xxxx\")")
	...
	ast.Inspect(f, func(n ast.Node) bool {
		...
		case *ast.FuncDecl:
			...
			fmt.Printf("Function %s starts at line %d, column %d end at line %d, column %d\n", x.Name.Name,
				pos.Line, pos.Column, end.Line, end.Column)

			if end.Line-pos.Line > 3 {
				var preLog []ast.Expr
				var afterLog []ast.Expr
				for _, param := range x.Type.Params.List {
					for _, ident := range param.Names {
						fmt.Printf("incoming  %s %s\n", ident.Name, param.Type)
						preLog = append(preLog, ident)
					}
				}
				if x.Type.Results != nil && len(x.Type.Results.List) > 0 {
					for _, result := range x.Type.Results.List {
						// 注意：这里假设每个结果只有一个标识符（即没有命名返回值的情况）
						// 如果有命名返回值，你可能需要额外处理
						for _, ident := range result.Names {
							fmt.Printf("outting  %s %s\n", ident.Name, result.Type)
							afterLog = append(afterLog, ident)
						}
				...
				//payloadExprLog, _ := parser.ParseExpr(fmt.Sprintf(`log.Println(%s,%s)`,
				//	strings.Repeat("%s ", len(preLog)), strings.Join(preLog, ",")))
				payloadExprPStmt := &ast.ExprStmt{
					X: &ast.CallExpr{
				...
				stmts := make([]ast.Stmt, 0, len(x.Body.List)+1)
				stmts = append(stmts, payloadExprPStmt)
				stmts = append(stmts, x.Body.List[:len(x.Body.List)-1]...)
				stmts = append(stmts, payloadExprAStmt)
				stmts = append(stmts, x.Body.List[len(x.Body.List)-1])
				x.Body.List = stmts
			}
		}
...
func mergeImportcfg(originalCfg, cfg string) error {
	...
	for name, line := range tempPkgs {
		//log.Println("originalPkgs dependence ", name, line)
